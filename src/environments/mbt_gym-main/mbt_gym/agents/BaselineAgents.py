...
class RandomAgent(Agent):
    ...
    def get_action(self, state: np.ndarray) -> np.ndarray:
        return np.repeat(
            self.action_space.sample().reshape(1, -1), self.num_trajectories, axis=0
        )
...
class FixedActionAgent(Agent):
    ...
    def get_action(self, state: np.ndarray) -> np.ndarray:
        return np.repeat(
            self.fixed_action.reshape(1, -1), self.env.num_trajectories, axis=0
        )
...
class FixedSpreadAgent(Agent):
    ...
    def get_action(self, state: np.ndarray) -> np.ndarray:
        action = np.array(
            [[self.half_spread - self.offset, self.half_spread + self.offset]]
        )
...
class HumanAgent(Agent):
    def get_action(self, state: np.ndarray):
        bid = float(
            input(
                f"Current state is {state}. How large do you want to set midprice-bid half spread? "
            )
        )
        ask = float(
            input(
                f"Current state is {state}. How large do you want to set ask-midprice half spread? "
            )
        )
...
class AvellanedaStoikovAgent(Agent):
    ...
    def _get_spread(self, time: float) -> float:
        ...
        volatility_aversion_component = (
            self.risk_aversion * self.volatility**2 * (self.terminal_time - time)
        )
        fill_exponent_component = (
            2 / self.risk_aversion * np.log(1 + self.risk_aversion / self.fill_exponent)
        )
        return volatility_aversion_component + fill_exponent_component

    def _get_action(self, inventory: int, time: float):
        bid_half_spread = (
            self._get_price_adjustment(inventory, time) + self._get_spread(time) / 2
        ).reshape(-1, 1)
        ask_half_spread = (
            -self._get_price_adjustment(inventory, time) + self._get_spread(time) / 2
        ).reshape(-1, 1)
...
class AvellanedaStoikovAgentModified(Agent):
    def __init__(self, risk_aversion: float = 0.1, env: TradingEnvironment = None):
        """
        modelled params:
        terminal_time: ???
        volatility: volatility of underlying for given asset
        rate_of_arrival:  arrival of orders to orderbook, this must be calculated somehow
        fill_exponent:  parameter of fill probability model, this must be calculated somehow

        Modifying agent from original paper to work with our environment requires moving from modelled inputs of volatility, intensity and fill_exponent to actual inputs.
        This is done by recalculating these parameters from the raw data that we are working with and updating it periodically.
    ...
    def update_params(self):
        ...
        self.rate_of_arrival = self._calculate_rate_of_arrival()
        self.fill_exponent = self._calculate_fill_exponent()

    def _calculate_volatility(self):
        pass

    def _calculate_rate_of_arrival(self):
        pass

    def _calculate_fill_exponent(self):
        pass
...
class CarteaJaimungalMmAgent(Agent):
    ...
    def _calculate_omega(self, current_time: float):
        ...
        return np.matmul(
            expm(self.a_matrix * (self.terminal_time - current_time)), self.z_vector
        )
...
class CarteaJaimungalOeAgent(Agent):
    def __init__(
        ...
        self.terminal_time = self.env.terminal_time
        self.temporary_price_impact = (
            self.price_impact_model.temporary_impact_coefficient
        )
        self.permanent_price_impact = (
            self.price_impact_model.permanent_impact_coefficient
        )
        self.num_trajectories = self.env.num_trajectories
    ...
    def get_action(self, state: np.ndarray):
        ...
        zeta = (
            self.alpha
            - 0.5 * self.permanent_price_impact
            + np.sqrt(self.temporary_price_impact * self.phi)
        )
        zeta /= (
            self.alpha
            - 0.5 * self.permanent_price_impact
        ...
        action[:, :] = (
            ...
            * (
                ...
                / (
                    zeta * np.exp(gamma * self.terminal_time)
                    - np.exp(-gamma * self.terminal_time)
                )
            )
        )
